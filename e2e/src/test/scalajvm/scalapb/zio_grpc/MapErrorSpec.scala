...
object MapErrorSpec extends ZIOSpecDefault {
    object ServerWithCustomError extends GTestService[Any, CustomError] {
        ...
        def unaryTypeMapped(request: Request, context: Any): IO[CustomError,WrappedString] = ZIO.fail(CustomError(request.in))

        def serverStreaming(request: Request, context: Any): Stream[CustomError,Response] = ZStream(Response("1"), Response("2")) ++ ZStream.fail(CustomError(request.in))
        ...
        def bidiStreaming(request: zio.stream.Stream[StatusException,Request], context: Any): zio.stream.Stream[CustomError,Response] = ???
    }
  ...
  val mappedServer = ServerWithCustomError.mapError(ce => new StatusException(Status.fromCodeValue(ce.code)))
    ...
    def spec = suite("MapErrorSpec")(
        ...
        test("maps errors for unary type mapped requests requests")(
          assertZIO(mappedServer.unaryTypeMapped(Request(in = 5), 0).exit)(fails(TestUtils.hasStatusCode(Status.fromCodeValue(5))))
        ),
