...
import zio.{Scope, Tag, Task, UIO, URIO, ZIO}
import zio.ZLayer
...
object ManagedServer {
  ...
  def fromServices[R1, S1, R2, S2, R3, S3](builder: => ServerBuilder[_], s1: S1, s2: S2, s3: S3)(implicit
    ...
    fromServiceList[R1 with R2 with R3](builder, ServiceList.add(s1).add(s2).add(s3))

  def fromServiceList[R](
      builder: => ServerBuilder[_],
      services: URIO[R, List[ServerServiceDefinition]]
  ): ZIO[R with Scope, Throwable, Server.Service] = fromServiceList(builder, services)
