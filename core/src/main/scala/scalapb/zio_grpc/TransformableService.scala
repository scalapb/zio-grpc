...
trait TransformableService[S[_, _]] {
  ...
  def toLayer[R, C: Tag](
      s: S[R, C]
  )(implicit tag: Tag[S[Any, C]]): ZLayer[R, Nothing, S[Any, C]] =
...
object TransformableService {
  ...
  final class TransformableServiceOps[S[_, _], R, C](private val service: S[R, C]) extends AnyVal {
    ...
    )(implicit TS: TransformableService[S], tag: Tag[C]): S[R0, C2] =
      TS.transformContextM[R, C, R0, C2](service, f)

    def provideEnvironment(
        r: ZEnvironment[R]
    )(implicit TS: TransformableService[S], tag: Tag[C]): S[Any, C] =
