...
trait TransformableService[S[_, _]] {
  ...
  def transformContextM[R, FromContext: Tag: IsNotIntersection, R0 <: R, ToContext: Tag: IsNotIntersection](
      s: S[R, FromContext],
  ...
  def transformContext[R, FromContext: Tag: IsNotIntersection, ToContext: Tag: IsNotIntersection](
      s: S[R, FromContext],
  ...
  def toLayer[R, C: Tag](
      s: S[R, C]
  )(implicit tagged: Tag[S[Any, C]], notis: IsNotIntersection[S[Any, C]]): ZLayer[R, Nothing, S[Any, C]] =
    ZLayer.fromFunction((r: ZEnvironment[R]) => provideEnvironment(s, r))
}
...
object TransformableService {
  ...
  final class TransformableServiceOps[S[_, _], R, C](private val service: S[R, C]) extends AnyVal {
    ...
    def transformContextM[C2: Tag: IsNotIntersection, R0 <: R](
        f: C2 => ZIO[R0, Status, C]
    )(implicit TS: TransformableService[S], cTagged: Tag[C], cNotIs: IsNotIntersection[C]): S[R0, C2] =
    ...
    def toLayer(implicit
        ...
        sTagged: Tag[S[Any, C]],
        sNotIs: IsNotIntersection[S[Any, C]],
        cTagged: Tag[C]
